...
class bettercontrol(Gtk.Window):
    def __init__(self):
        ...
        wifi_header = Gtk.HeaderBar()
        wifi_header.set_show_close_button(False)
        wifi_header.set_title("Wi-Fi Networks")
        wifi_header.set_subtitle("Available networks and connection status")
        wifi_box.pack_start(wifi_header, False, False, 0)

        self.wifi_grid = Gtk.Grid()
        self.wifi_grid.set_column_spacing(10)
        self.wifi_grid.set_row_spacing(10)
        self.wifi_grid.set_margin_top(10)
        self.wifi_grid.set_margin_bottom(10)
        self.wifi_grid.set_margin_start(10)
        self.wifi_grid.set_margin_end(10)
        wifi_box.pack_start(self.wifi_grid, True, True, 0)

        wifi_button_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        wifi_button_box.set_halign(Gtk.Align.CENTER)
        wifi_button_box.set_valign(Gtk.Align.END)
        wifi_button_box.set_margin_top(10)
        wifi_button_box.set_margin_bottom(10)
        wifi_button_box.set_margin_start(10)
        wifi_button_box.set_margin_end(10)

        refresh_wifi_button = Gtk.Button()
        refresh_wifi_button.set_image(Gtk.Image.new_from_icon_name("view-refresh", Gtk.IconSize.BUTTON))
        refresh_wifi_button.set_tooltip_text("Refresh network list")
        refresh_wifi_button.connect("clicked", self.refresh_wifi)
        wifi_button_box.pack_start(refresh_wifi_button, False, False, 0)

        connect_wifi_button = Gtk.Button()
        connect_wifi_button.set_image(Gtk.Image.new_from_icon_name("network-wireless", Gtk.IconSize.BUTTON))
        connect_wifi_button.set_tooltip_text("Connect to selected network")
        connect_wifi_button.connect("clicked", self.connect_wifi)
        wifi_button_box.pack_start(connect_wifi_button, False, False, 0)

        forget_wifi_button = Gtk.Button()
        forget_wifi_button.set_image(Gtk.Image.new_from_icon_name("edit-delete", Gtk.IconSize.BUTTON))
        forget_wifi_button.set_tooltip_text("Forget selected network")
        forget_wifi_button.connect("clicked", self.forget_wifi)
        wifi_button_box.pack_start(forget_wifi_button, False, False, 0)

        disconnect_wifi_button = Gtk.Button()
        disconnect_wifi_button.set_image(Gtk.Image.new_from_icon_name("network-offline", Gtk.IconSize.BUTTON))
        disconnect_wifi_button.set_tooltip_text("Disconnect from current network")
        ...
        wifi_box.pack_start(password_box, False, False, 0)
        ...
        speed_box.pack_start(self.download_label, False, False, 0)
        speed_box.pack_start(self.upload_label, False, False, 0)
    ...
    def refresh_wifi(self, button):
        self.wifi_grid.foreach(lambda child: self.wifi_grid.remove(child))
        networks = subprocess.getoutput("nmcli dev wifi").split("\n")[1:]
        
        headers = ["SSID", "Security", "Signal", "Rate", "In Use"]
        for i, header in enumerate(headers):
            label = Gtk.Label(label=header)
            label.set_markup(f"<b>{header}</b>")
            label.set_xalign(0)
            self.wifi_grid.attach(label, i, 0, 1, 1)

        for row, network in enumerate(networks, start=1):
            ...
            signal = parts[2]
            rate = parts[3]
            in_use = "✓" if "*" in parts else ""

            for col, value in enumerate([ssid, security, signal, rate, in_use]):
                label = Gtk.Label(label=value)
                label.set_xalign(0)
                
                if col == 0:  # SSID
                    label.set_markup(f"<b>{value}</b>")
                elif col == 2:  # Signal
                    if int(value) < 30:
                        label.set_markup(f"<span foreground='red'>{value}</span>")
                    elif int(value) < 60:
                        label.set_markup(f"<span foreground='orange'>{value}</span>")
                elif col == 4 and value == "✓":  # In Use
                    label.set_markup(f"<span foreground='green'>{value}</span>")

                self.wifi_grid.attach(label, col, row, 1, 1)

        self.wifi_grid.show_all()
    ...
    def _update_wifi_list(self, networks):
        self.wifi_grid.foreach(lambda child: self.wifi_grrid.remove(child))
    ...
    def connect_wifi(self, button):
        selected_row = self.wifi_grid.get_selected_row()
        if not selected_row:
