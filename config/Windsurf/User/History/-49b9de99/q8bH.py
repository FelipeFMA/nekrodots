...
class bettercontrol(Gtk.Window):
        ...
        def connect_thread():
            try:
                if is_secured:
                    ...
                    connect_args = ["nmcli", "dev", "wifi", "connect", ssid, "password", password]
                    if not remember:
                        # If user unchecked "Remember this network", add "--nowait" to prevent saving
                        connect_args.append("--nowait")
    ...
    def show_notification(self, message):
        ...
        close_button.connect("clicked", lambda btn: notification.set_reveal_child(False))
        box.pack_end(close_button, False, False, 0)
        
        frame = Gtk.Frame()
        frame.add(box)
        notification.add(frame)
        
        # Store the notification
        self._current_notification = notification
        
        # Add to the top of the main container
        self.main_container.pack_start(notification, False, False, 0)
        self.main_container.reorder_child(notification, 0)
        ...
        def remove_notification():
            try:
                if notification.get_parent() == self.main_container:
                    self.main_container.remove(notification)
            except:
                pass  # Already removed or container changed
            
            # Clear current notification if it matches
            if hasattr(self, '_current_notification') and self._current_notification == notification:
                self._current_notification = None
                
            return False  # Stop the timeout
        
        GLib.timeout_add_seconds(5, hide_notification)
        # Remove from container after hiding animation completes
        GLib.timeout_add_seconds(6, remove_notification)
