...
class HyprlandSettingsApp(Gtk.Window):
    def __init__(self):
        ...
        # Refresh all categories when the app starts
        self.refresh_wifi(None)
        # Check if bluetoothctl is available before refreshing Bluetooth
    ...
    def refresh_bluetooth(self, button):
        ...
        if not shutil.which("bluetoothctl"):
            row = Gtk.ListBoxRow()
            label = Gtk.Label(label="BlueZ is not installed. Install it with: sudo pacman -S bluez bluez-utils")
            row.add(label)
        ...
        if bt_status != "active":
            row = Gtk.ListBoxRow()
            label = Gtk.Label(label="Bluetooth is disabled. Enable it first.")
            row.add(label)
            self.bt_listbox.add(row)
            self.bt_listbox.show_all()
            return
        ...
        try:
            ...
            if not devices or devices == [""]:
                row = Gtk.ListBoxRow()
                label = Gtk.Label(label="No Bluetooth devices found nearby.")
                row.add(label)
                self.bt_listbox.add(row)
                self.bt_listbox.show_all()
                return
