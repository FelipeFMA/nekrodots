...
class WiFiNetworkRow(Gtk.ListBoxRow):
    def __init__(self, network_info):
        ...
        self.signal_strength = next((p for p in parts if p.endswith('%')), "0%")
        
        # Determine signal icon based on signal strength percentage
        signal_value = int(self.signal_strength.rstrip('%')) if self.signal_strength.rstrip('%').isdigit() else 0
...
class bettercontrol(Gtk.Window):
    ...
    def __init__(self):
        ...
        css_data = """
            ...
            .wifi-action-button {
                border-radius: 5px;
                padding: 5px 10px;
                background-color: #3498db;↪ nmcli con add type wifi con-name Capivara-2GHz ssid Capivara-2GHz wifi-sec.key-mgmt wpa-psk wifi-sec.
    ...
    def _refresh_wifi_thread(self):
        networks = subprocess.getoutput("nmcli -t -f ACTIVE,SSID,SIGNAL,SECURITY dev wifi list").split("\n")
    ...
    def _update_wifi_list(self, networks):
        ...
        # Also get the full information for display in the UI
        full_networks = subprocess.getoutput("nmcli dev wifi list").split("\n")[1:]
    ...
    def forget_wifi(self, button):
        selected_row = self.wifi_listbox.get_selected_row()
        if not selected_row:
            self.show_notification("Please select a Wi-Fi network to forget")
            return
        ...
        try:
            ...
            if not connections or not connections[0]:
                self.show_notification(f"No saved connection found for '{ssid}'")
                return
                
            uuid = connections[0].split(":")[1]
            subprocess.run(["nmcli", "connection", "delete", "uuid", uuid], check=True)
            self.show_notification(f"Successfully forgot network '{ssid}'")
            self.refresh_wifi(None)
        except subprocess.CalledProcessError as e:
            self.show_notification(f"Failed to forget network: {e}")
    ...
    def disconnect_wifi(self, button):
        try:
            ...
            if not wifi_interfaces:
                self.show_notification("No Wi-Fi interface found")
                return
                
            for interface in wifi_interfaces:
                subprocess.run(["nmcli", "dev", "disconnect", interface], check=True)
                
            self.show_notification("Disconnected from Wi-Fi network")
            self.refresh_wifi(None)
        except subprocess.CalledProcessError as e:
            self.show_notification(f"Failed to disconnect: {e}")
    ...
    def show_notification(self, message):
        """Display a notification message that auto-hides after a few seconds."""
        # Check if we have an existing notification
        if hasattr(self, '_current_notification') and self._current_notification:
            # If it's still in the container, remove it
            try:
                self.main_container.remove(self._current_notification)
            except:
                pass  # It might have been already removed
        
        # Create a fixed container for the notification
        ...
        # Set the type based on the message content
        if "Failed" in message or "Error" in message or "failed" in message or "error" in message:
            notification.set_message_type(Gtk.MessageType.ERROR)
        elif "Success" in message or "Connected" in message or "success" in message:
            notification.set_message_type(Gtk.MessageType.INFO)
        else:
            notification.set_message_type(Gtk.MessageType.WARNING)
            
        # Get the content area
        content_area = notification.get_content_area()
        ...
        # Add close button
        notification.add_button("_Close", Gtk.ResponseType.CLOSE)
        notification.connect("response", lambda info_bar, response_id: notification_box.destroy())
        
        # Add the notification to the container
        notification_box.pack_start(notification, False, False, 0)
        
        # Store the notification
        self._current_notification = notification_box
        
        # Add to the top of the main container
        self.main_container.pack_start(notification_box, False, False, 0)
        self.main_container.reorder_child(notification_box, 0)
        
        notification_box.show_all()
        
        # Auto-hide after 15 seconds (increased from 10 seconds)
        def hide_notification():
            try:
                if notification_box.get_parent() == self.main_container:
                    self.main_container.remove(notification_box)
                    # Clear current notification if it matches
                    if hasattr(self, '_current_notification') and self._current_notification == notification_box:
                        self._current_notification = None
            except:
                pass  # Already removed or container changed
            return False  # Stop the timeout
            
        # Increase timeout to 15 seconds for better readability
        GLib.timeout_add_seconds(15, hide_notification)
    ...
    def connect_wifi(self, button):
        ...
        selected_row = self.wifi_listbox.get_selected_row()
        if not selected_row:
            self.show_notification("Please select a Wi-Fi network to connect")
            return
                ...
                if is_secured:
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            time.sleep(2)
                            GLib.idle_add(lambda: self.show_notification(f"Successfully connected to {ssid}"))
                        else:
                            print(f"Error code: {result.returncode}")
                            print(f"Error output: {result.stderr}")
                            error_msg = result.stderr if result.stderr else f"Error code: {result.returncode}"
                            GLib.idle_add(lambda: self.show_notification(f"Failed to connect: {error_msg}"))
                            
                    except Exception as e:
                        print(f"Exception connecting to network: {e}")
                        GLib.idle_add(lambda: self.show_notification(f"Error connecting: {str(e)}"))
                else:
                    print(f"Connecting to open network: {ssid}")
                    try:
                        # Use simpler command for non-secured networks
                        result = subprocess.run(
