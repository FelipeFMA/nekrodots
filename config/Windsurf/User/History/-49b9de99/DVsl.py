...
class WiFiNetworkRow(Gtk.ListBoxRow):
    def __init__(self, network_info):
        ...
        # Look for a numeric value in the SIGNAL column (7th column in standard nmcli output)
        signal_value = 0
        try:
            # Standard nmcli output has signal in column 7 (index 6)
            if len(parts) > 6 and parts[6].isdigit():
                signal_value = int(parts[6])
                self.signal_strength = f"{signal_value}%"
            else:
                # Fallback for tabular output: find a value that looks like it could be signal strength
                for p in parts:
