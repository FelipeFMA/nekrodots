...
class bettercontrol(Gtk.Window):
    ...
    def __init__(self):
        ...
        # Add pairing mode button
        pairing_button = Gtk.Button()
        pairing_button.set_tooltip_text("Enable Pairing Mode")
        pairing_icon = Gtk.Image.new_from_icon_name("bluetooth-active-symbolic", Gtk.IconSize.BUTTON)
        pairing_button.add(pairing_icon)
        pairing_button.connect("clicked", self.enable_pairing_mode)
        refresh_box.pack_end(pairing_button, False, False, 5)
        
        action_box.pack_end(refresh_box, True, True, 0)
        
        bluetooth_box.pack_start(action_box, False, False, 0)
        
        # Bluetooth devices list with scrolling
        scroll_window = Gtk.ScrolledWindow()
        scroll_window.set_policy(Gtk.PolicyType.NEVER, Gtk.PolicyType.AUTOMATIC)
        scroll_window.set_vexpand(True)
        ...
        # Scanning indicator
        self.bt_scanning_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        self.bt_scanning_box.set_margin_top(5)
        self.bt_scanning_box.set_no_show_all(True)
        
        self.bt_spinner = Gtk.Spinner()
        self.bt_scanning_box.pack_start(self.bt_spinner, False, False, 0)
        
        scanning_label = Gtk.Label(label="Scanning for devices...")
        scanning_label.get_style_context().add_class("dim-label")
        self.bt_scanning_box.pack_start(scanning_label, False, False, 0)
        
        bluetooth_box.pack_start(self.bt_scanning_box, False, False, 0)
        ...
        connect_button = Gtk.Button(label="Connect")
        connect_button.get_style_context().add_class("wifi-action-button")
        connect_button.connect("clicked", self.connect_selected_bluetooth_device)
        button_box.pack_start(connect_button, True, True, 0)
        
        disconnect_button = Gtk.Button(label="Disconnect")
        disconnect_button.connect("clicked", self.disconnect_selected_bluetooth_device)
        button_box.pack_start(disconnect_button, True, True, 0)
        
        forget_button = Gtk.Button(label="Forget Device")
        forget_button.connect("clicked", self.forget_selected_bluetooth_device)
        button_box.pack_start(forget_button, True, True, 0)
        
        bluetooth_box.pack_start(button_box, False, False, 0)

        self.tabs["Bluetooth"] = bluetooth_box
        if self.tab_visibility.get("Bluetooth", True):  
            self.notebook.append_page(bluetooth_box, Gtk.Label(label="Bluetooth"))
            # Initialize Bluetooth status in a background thread
            thread = threading.Thread(target=self._init_bluetooth_tab)
            thread.daemon = True
            thread.start()
        ...
        # Header with Volume title and icons
        header_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=10)
        header_box.set_margin_bottom(10)
        ...
        volume_header_label = Gtk.Label(label="Audio Controls")
        volume_header_label.get_style_context().add_class("wifi-header")
        header_box.pack_start(volume_header_label, False, False, 0)
        
        volume_box.pack_start(header_box, False, False, 0)
        
        # Speaker section
        speaker_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        speaker_section.set_margin_top(10)
        speaker_section.set_margin_bottom(10)
        
        # Speaker header
        speaker_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        speaker_icon = Gtk.Image.new_from_icon_name("audio-speakers-symbolic", Gtk.IconSize.LARGE_TOOLBAR)
        speaker_header.pack_start(speaker_icon, False, False, 0)
        
        volume_label = Gtk.Label(label="Speaker Volume")
        volume_label.set_halign(Gtk.Align.START)
        volume_label.set_markup("<b>Speaker Volume</b>")
        speaker_header.pack_start(volume_label, False, False, 0)
        
        self.volume_button = Gtk.Button(label="Mute/Unmute")
        self.volume_button.connect("clicked", self.mute)
        self.volume_button.get_style_context().add_class("wifi-action-button")
        ...
        self.volume_scale.set_margin_top(5)
        self.volume_scale.set_margin_bottom(5)
        speaker_section.pack_start(self.volume_scale, False, False, 0)
        
        # Speaker preset buttons
        ...
        preset_box.pack_start(self.volume_zero, True, True, 0)
        
        self.volume_tfive = Gtk.Button(label="25%")
        self.volume_tfive.connect("clicked", self.vtfive)
        preset_box.pack_start(self.volume_tfive, True, True, 0)
        
        self.volume_fifty = Gtk.Button(label="50%")
        self.volume_fifty.connect("clicked", self.vfifty)
        preset_box.pack_start(self.volume_fifty, True, True, 0)
        
        self.volume_sfive = Gtk.Button(label="75%")
        self.volume_sfive.connect("clicked", self.vsfive)
        preset_box.pack_start(self.volume_sfive, True, True, 0)
        
        self.volume_hund = Gtk.Button(label="100%")
        self.volume_hund.connect("clicked", self.vhund)
        preset_box.pack_start(self.volume_hund, True, True, 0)
        
        speaker_section.pack_start(preset_box, False, False, 0)
        volume_box.pack_start(speaker_section, False, False, 0)
        
        # Separator
        separator = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
        separator.set_margin_top(10)
        separator.set_margin_bottom(10)
        volume_box.pack_start(separator, False, False, 0)
        
        # Microphone section
        mic_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        mic_section.set_margin_top(10)
        mic_section.set_margin_bottom(10)
        
        # Microphone header
        mic_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        mic_icon = Gtk.Image.new_from_icon_name("audio-input-microphone-symbolic", Gtk.IconSize.LARGE_TOOLBAR)
        mic_header.pack_start(mic_icon, False, False, 0)
        
        mic_label = Gtk.Label(label="Microphone Volume")
        mic_label.set_halign(Gtk.Align.START)
        mic_label.set_markup("<b>Microphone Volume</b>")
        mic_header.pack_start(mic_label, False, False, 0)
        
        self.volume_mic = Gtk.Button(label="Mute/Unmute")
        self.volume_mic.connect("clicked", self.micmute)
        self.volume_mic.get_style_context().add_class("wifi-action-button")
        ...
        self.mic_scale.set_margin_top(5)
        self.mic_scale.set_margin_bottom(5)
        mic_section.pack_start(self.mic_scale, False, False, 0)
        
        # Microphone preset buttons
        mic_preset_box = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        mic_preset_box.set_homogeneous(True)
        
        self.mic_zero = Gtk.Button(label="0%")
        self.mic_zero.connect("clicked", self.mzero)
        mic_preset_box.pack_start(self.mic_zero, True, True, 0)
        
        self.mic_tfive = Gtk.Button(label="25%")
        self.mic_tfive.connect("clicked", self.mtfive)
        mic_preset_box.pack_start(self.mic_tfive, True, True, 0)
        
        self.mic_fifty = Gtk.Button(label="50%")
        self.mic_fifty.connect("clicked", self.mfifty)
        mic_preset_box.pack_start(self.mic_fifty, True, True, 0)
        
        self.mic_sfive = Gtk.Button(label="75%")
        self.mic_sfive.connect("clicked", self.msfive)
        mic_preset_box.pack_start(self.mic_sfive, True, True, 0)
        
        self.mic_hund = Gtk.Button(label="100%")
        self.mic_hund.connect("clicked", self.mhund)
        mic_preset_box.pack_start(self.mic_hund, True, True, 0)
        
        mic_section.pack_start(mic_preset_box, False, False, 0)
        volume_box.pack_start(mic_section, False, False, 0)
        
        # Separator
        separator2 = Gtk.Separator(orientation=Gtk.Orientation.HORIZONTAL)
        separator2.set_margin_top(10)
        separator2.set_margin_bottom(10)
        volume_box.pack_start(separator2, False, False, 0)
        
        # Output device selection
        device_section = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=10)
        device_section.set_margin_top(10)
        
        device_header = Gtk.Box(orientation=Gtk.Orientation.HORIZONTAL, spacing=5)
        device_icon = Gtk.Image.new_from_icon_name("audio-card-symbolic", Gtk.IconSize.LARGE_TOOLBAR)
        device_header.pack_start(device_icon, False, False, 0)
        
        device_label = Gtk.Label(label="Output Device")
        device_label.set_halign(Gtk.Align.START)
        device_label.set_markup("<b>Output Device</b>")
        device_header.pack_start(device_label, False, False, 0)
        
        device_section.pack_start(device_header, False, False, 0)
        
        self.sink_dropdown = Gtk.ComboBoxText()
        self.sink_dropdown.connect("changed", self.on_sink_selected)
        self.sink_dropdown.set_margin_top(5)
        device_section.pack_start(self.sink_dropdown, False, False, 0)
        
        volume_box.pack_start(device_section, False, False, 0)
    ...
    def update_bluetooth_status(self):
        """Update the Bluetooth status switch based on the actual Bluetooth state."""
        try:
            bt_status = subprocess.run(
                ["systemctl", "is-active", "bluetooth"], capture_output=True, text=True
            ).stdout.strip()
            GLib.idle_add(lambda: self.bt_status_switch.set_active(bt_status == "active"))
        except Exception as e:
    ...
    def on_bluetooth_switch_toggled(self, switch, gparam):
        ...
        active = switch.get_active()
        if active:
            thread = threading.Thread(target=self._enable_bluetooth_thread)
            thread.daemon = True
            thread.start()
        else:
            thread = threading.Thread(target=self._disable_bluetooth_thread)
            thread.daemon = True
            thread.start()

    def _enable_bluetooth_thread(self):
        """Enable Bluetooth in a background thread."""
        if not shutil.which("bluetoothctl"):
            GLib.idle_add(lambda: self.show_error_dialog("BlueZ is not installed. Install it with:\n\nsudo pacman -S bluez bluez-utils"))
            return
        ...
        GLib.idle_add(lambda: self.show_error_dialog("Failed to enable Bluetooth. Make sure BlueZ is installed."))

    def _disable_bluetooth_thread(self):
        """Disable Bluetooth in a background thread."""
        print("Disabling Bluetooth...")
        subprocess.run(["systemctl", "stop", "bluetooth"])
        print("Bluetooth disabled.")
        GLib.idle_add(lambda: self.bt_listbox.foreach(lambda row: self.bt_listbox.remove(row)))
        GLib.idle_add(lambda: self.bt_listbox.show_all())

    def enable_bluetooth(self, button):
        """Start a thread to enable Bluetooth."""
        thread = threading.Thread(target=self._enable_bluetooth_thread)
        thread.daemon = True
        thread.start()

    def disable_bluetooth(self, button):
        """Start a thread to disable Bluetooth."""
        thread = threading.Thread(target=self._disable_bluetooth_thread)
        thread.daemon = True
        thread.start()

    def enable_pairing_mode(self, button):
        """Enable Bluetooth pairing mode in a background thread."""
        thread = threading.Thread(target=self._enable_pairing_mode_thread)
        thread.daemon = True
        thread.start()

    def _enable_pairing_mode_thread(self):
        """Enable Bluetooth pairing mode in a background thread."""
        try:
            # Show scanning indicator
            GLib.idle_add(lambda: self.bt_scanning_box.show())
            GLib.idle_add(lambda: self.bt_spinner.start())
            ...
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())
            
            # Refresh the device list
            GLib.idle_add(self.refresh_bluetooth, None)
        except Exception as e:
            print(f"Error enabling pairing mode: {e}")
            GLib.idle_add(lambda: self.show_error_dialog(f"Error enabling pairing mode: {e}"))
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())

    def connect_selected_bluetooth_device(self, button):
        """Connect to the selected Bluetooth device."""
        selected_row = self.bt_listbox.get_selected_row()
        if not selected_row:
            return
            
        # Extract the MAC address from the row
        mac_address = None
        for child in selected_row.get_children():
            if isinstance(child, Gtk.Box):
                for box_child in child.get_children():
                    if isinstance(box_child, Gtk.Label):
                        # The MAC address is stored in the row data
                        mac_address = selected_row.mac_address
                        break
        
        if mac_address:
            self.connect_bluetooth_device(button, mac_address)

    def disconnect_selected_bluetooth_device(self, button):
        ...
        if mac_address:
            self.disconnect_bluetooth_device(button, mac_address)

    def forget_selected_bluetooth_device(self, button):
        ...
        if mac_address:
            self.forget_bluetooth_device(button, mac_address)

    def connect_bluetooth_device(self, button, mac_address):
        """Connect to a Bluetooth device in a background thread."""
        thread = threading.Thread(target=self._connect_bluetooth_device_thread, args=(mac_address,))
        thread.daemon = True
        thread.start()

    def _connect_bluetooth_device_thread(self, mac_address):
        """Connect to a Bluetooth device in a background thread."""
        try:
            # Show scanning indicator
            GLib.idle_add(lambda: self.bt_scanning_box.show())
            ...
            pair_result = subprocess.run(["bluetoothctl", "pair", mac_address], capture_output=True, text=True)
            
            # Then connect
            connect_result = subprocess.run(["bluetoothctl", "connect", mac_address], capture_output=True, text=True)
            ...
            if connect_result.returncode == 0:
                ...
                error_msg = connect_result.stderr if connect_result.stderr else "Unknown error"
                print(f"Error connecting to {mac_address}: {error_msg}")
                GLib.idle_add(lambda: self.show_error_dialog(f"Error connecting to device: {error_msg}"))
            
            # Refresh the device list
            GLib.idle_add(self.refresh_bluetooth, None)
        except Exception as e:
            print(f"Error connecting to {mac_address}: {e}")
            GLib.idle_add(lambda: self.show_error_dialog(f"Error connecting to device: {e}"))
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())

    def disconnect_bluetooth_device(self, button, mac_address):
        """Disconnect from a Bluetooth device in a background thread."""
        thread = threading.Thread(target=self._disconnect_bluetooth_device_thread, args=(mac_address,))
        thread.daemon = True
        thread.start()

    def _disconnect_bluetooth_device_thread(self, mac_address):
        """Disconnect from a Bluetooth device in a background thread."""
        try:
            # Show scanning indicator
            GLib.idle_add(lambda: self.bt_scanning_box.show())
            ...
            # Disconnect
            disconnect_result = subprocess.run(["bluetoothctl", "disconnect", mac_address], capture_output=True, text=True)
            ...
            if disconnect_result.returncode == 0:
                print(f"Successfully disconnected from {mac_address}")
            else:
                error_msg = disconnect_result.stderr if disconnect_result.stderr else "Unknown error"
                print(f"Error disconnecting from {mac_address}: {error_msg}")
                GLib.idle_add(lambda: self.show_error_dialog(f"Error disconnecting from device: {error_msg}"))
            
            # Refresh the device list
            GLib.idle_add(self.refresh_bluetooth, None)
        except Exception as e:
            print(f"Error disconnecting from {mac_address}: {e}")
            GLib.idle_add(lambda: self.show_error_dialog(f"Error disconnecting from device: {e}"))
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())

    def forget_bluetooth_device(self, button, mac_address):
        """Forget a Bluetooth device in a background thread."""
        thread = threading.Thread(target=self._forget_bluetooth_device_thread, args=(mac_address,))
        thread.daemon = True
        thread.start()

    def _forget_bluetooth_device_thread(self, mac_address):
        """Forget a Bluetooth device in a background thread."""
        try:
            # Show scanning indicator
            GLib.idle_add(lambda: self.bt_scanning_box.show())
            ...
            # Remove device
            remove_result = subprocess.run(["bluetoothctl", "remove", mac_address], capture_output=True, text=True)
            ...
            if remove_result.returncode == 0:
                ...
                error_msg = remove_result.stderr if remove_result.stderr else "Unknown error"
                print(f"Error removing device {mac_address}: {error_msg}")
                GLib.idle_add(lambda: self.show_error_dialog(f"Error removing device: {error_msg}"))
            
            # Refresh the device list
            GLib.idle_add(self.refresh_bluetooth, None)
        except Exception as e:
            print(f"Error removing device {mac_address}: {e}")
            GLib.idle_add(lambda: self.show_error_dialog(f"Error removing device: {e}"))
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())
        
    def refresh_bluetooth(self, button):
        """Refreshes the list of Bluetooth devices (paired + nearby) in a background thread."""
        bt_status = subprocess.run(
            ["systemctl", "is-active", "bluetooth"], capture_output=True, text=True
        ).stdout.strip()

        if bt_status != "active":
            self.show_error_dialog("Bluetooth is disabled. Enable it first.")
            return

        # Show scanning indicator
        self.bt_scanning_box.show()
    ...
    def _refresh_bluetooth_thread(self):
        ...
        try:
            ...
            print(f"Error in refresh Bluetooth thread: {e}")
            GLib.idle_add(lambda: self.show_error_dialog(f"Error refreshing Bluetooth devices: {e}"))
            GLib.idle_add(lambda: self.bt_spinner.stop())
            GLib.idle_add(lambda: self.bt_scanning_box.hide())
    ...
    def _update_bluetooth_list(self, devices):
        ...
        # Hide scanning indicator
        self.bt_spinner.stop()
        self.bt_scanning_box.hide()
        ...
        for device in devices:
            ...
            try:
                ...
                is_paired = "Paired: yes" in status_output
                
                # Get device type/icon
                device_type = "default"
                if "Icon: audio-card" in status_output:
                    device_type = "audio"
                elif "Icon: audio-headset" in status_output or "Icon: audio-headphones" in status_output:
                    device_type = "headset"
                elif "Icon: input-keyboard" in status_output:
                    device_type = "keyboard"
                elif "Icon: input-mouse" in status_output:
                    device_type = "mouse"
                elif "Icon: input-gaming" in status_output:
                    device_type = "gamepad"
                elif "Icon: phone" in status_output:
                    device_type = "phone"
                
                # Create a styled row similar to WiFi networks
                row = Gtk.ListBoxRow()
                ...
                # Left side with device name and status
                left_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=3)
                ...
                print(f"Error processing device {mac_address}: {e}")
    ...
    def on_tab_switched(self, notebook, page, page_num):
        ...
        # If switching to Bluetooth tab, refresh in background
        if tab_label == "Bluetooth":
            # Only refresh if Bluetooth is enabled
            try:
                bt_status = subprocess.run(
                ...
                print(f"Error checking Bluetooth status on tab switch: {e}")

    def _init_bluetooth_tab(self):
        """Initialize the Bluetooth tab in a background thread."""
        try:
            ...
            # If Bluetooth is active, refresh the device list
            if bt_status == "active":
                # Show scanning indicator
                GLib.idle_add(lambda: self.bt_scanning_box.show())
                GLib.idle_add(lambda: self.bt_spinner.start())
                ...
                # Refresh devices
                self._refresh_bluetooth_thread()
        except Exception as e:
            print(f"Error initializing Bluetooth tab: {e}")
